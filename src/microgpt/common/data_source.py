...
class DatasetDataSource(BaseModel):
    ...
    def __str__(self) -> str:
        return f"DatasetDataSource(name={self.name}, params={self.params}, batch_size={self.batch_size}, max_length={self.max_length})"
...
async def _batch_read_data_source(data_source: DataSource) -> AsyncGenerator[str, None]:
    if data_source.type == "str":
        ...
        if data_source.max_length > 0 and (
            max_length is None or max_length > data_source.max_length
        ):
