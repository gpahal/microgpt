...
_CL100K_SPLIT_PATTERN = r"""'(?i:[sdmt]|ll|ve|re)|[^\r\n\p{L}\p{N}]?+\p{L}++|\p{N}{1,3}+| ?[^\s\p{L}\p{N}]++[\r\n]*+|\s++$|\s*[\r\n]|\s+(?!\S)|\s"""
...
async def _read_file_cached_internal(
    cache_dir: str, blobpath: str, expected_hash: str | None = None
) -> bytes:
    if cache_dir == "":
...
async def _load_mergeable_ranks_from_tiktoken_bpe(
    bpe_file_path: str, bpe_hash: str | None = None
) -> dict[bytes, int]:
