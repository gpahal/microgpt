    ...
    def crop_seq_len(self, max_seq_len: int) -> None:
        ...
        for block in self.transformer.h:  # type: ignore
            if hasattr(block.attn, "bias"):
                block.attn.bias = block.attn.bias[:, :, :max_seq_len, :max_seq_len]  # type: ignore
        ...
        if config.type == "untrained":
            ...
            if not os.path.exists(model_weights_file_path):
                raise FileNotFoundError(
                    f"Pretrained model weights file not found: model_weights_file_path={model_weights_file_path}"
                )

            file_size = os.path.getsize(model_weights_file_path)
            if file_size < 100 * 1024:
                raise ValueError(
                    f"Pretrained model file at {model_weights_file_path} is not fetched from GitLFS. "
                    "Please run `scripts/model/download_pretrained_model_weights.sh` to fetch the file."
                )
