...
class _RoPE(nn.Module):
    ...
    def _get_position_buffers(
        ...
        if self.positions_sin is not None and self.positions_cos is not None and seq_len <= self.positions_sin.size(2):
            if torch_device is not None and self.positions_sin.device != torch_device:
                self.positions_sin = self.positions_sin.to(torch_device)
                self.positions_cos = self.positions_cos.to(torch_device)
        ...
        with ctx:
            ...
            self.positions_sin = positions.sin()[None, None, :, :]  # (1, 1, seq_len, hs)
            self.positions_cos = positions.cos()[None, None, :, :]  # (1, 1, seq_len, hs)
