...
Command to train the pretrained model for 5 epochs. To download the training data from S3, see
`scripts/data/prepare_data.py`:

```sh
uv run python -m scripts.model.train_model \
    --epochs 5 \
...
```sh
uv run torchrun --standalone --nproc_per_node=8 scripts/model/train_model.py \
    --epochs 5 \
    --max-iterations-per-epoch 18170 \
    --batch-size 64 \
    --gradient-accumulation-iterations 8 \
...
async def train(
    ...
    trained_model_dir_path = os.path.join(dirname, "trained_model")
...
def main(
    checkpointing_interval: Annotated[
        ...
        typer.Option(
            ...
            help="The number of iterations between checkpoints. If not provided, it is set to min(10000, max(10, max_iterations // 100))",
            min=1,
        ),
    ] = None,
    epochs: Annotated[
    ...
    gradient_accumulation_iterations: Annotated[
        ...
        typer.Option(
            ...
            "-g",
            help="The number of gradient accumulation iterations. The model effectively trains on a batch size of batch_size * gradient_accumulation_iterations, with loss accumulated over the gradient_accumulation_iterations.",
        ),
    ] = 2,
